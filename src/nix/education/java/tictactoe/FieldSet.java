...
public class FieldSet {
    ...
    private void errorMssagesOutput(int ERROR_CODE) {
        if (ERROR_CODE == ticTacToeConstants.WRONG_CHARACTER_CODE) {
            System.out.println("Недопустимый символ!");
        }
        else if (ERROR_CODE == ticTacToeConstants.WRONG_LENGTH_CODE) {
            ...
            for (int x = 0; x < HORIZONTAL_LENGTH; x++ ) {
                for (int y = 0; y < VERTICAL_LENGTH; y++ ) {
                    if (checkInputSymbols(INPUT_STRING.charAt(counter))) {
                        ...
                        errorMssagesOutput(ticTacToeConstants.WRONG_CHARACTER_CODE);
                    }
                }
            }
        }
        else {
            errorMssagesOutput(ticTacToeConstants.WRONG_LENGTH_CODE);
        }
    }

    public void setTheSymbol(int x, int y, char[][] GAME_FIELD, char SYMBOL) {
